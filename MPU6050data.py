...
# Knop, om meting te starten (ingedrukt houden)

LED = machine.Pin(16, machine.Pin.OUT)
button = Pin(17, machine.Pin.IN,machine.Pin.PULL_DOWN)
file = open("Gkrachten.txt", "w")
...
while True:
    if button.value() == 1:
        ...
        utime.sleep(0.01)
        LED.value(1)
        file.write(str(mpu6050_get_accel(i2c)) + "\n")
        file.flush()
    else:
        print("druk op button om te starten")
        LED.value(0)
        utime.sleep(5)
